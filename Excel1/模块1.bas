...
'可用Auto_Open作为宏名，打开文件时自动运行
    ...
    Sheets("个人").Select
    Application.Run "小说统计.xlsm!自动2"
        
    Sheets("近期").Select
    Application.Run "小说统计.xlsm!自动3"
...
Sub Auto_Close()
    ...
    If InStr(1, ActiveWorkbook.Name, "小说统计.xlsm", vbBinaryCompare) > 0 Then
